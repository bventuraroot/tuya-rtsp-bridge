# API HTTP lokal (`http://127.0.0.1:8787`)

JSON masuk/keluar. Tanpa auth — bind lokal + LAN; jangan expose `:8787` ke internet.
...
UI statis: `GET /` dari `web/`. Dengan `cloud_auth.json` bisa password-relogin tanpa QR baru.
